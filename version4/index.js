...
function DitLaVoix() {
    recognition.addEventListener('result', function (event) {
        for (var i = event.resultIndex; i < event.results.length; i++) {
            ...
            if (event.results[i].isFinal && confidence >= confidenceThreshold && transcript.includes('Jarvis')) {
        ...
        if (choice.finish_reason == 'function_call' && choice.message.function_call.name == 'create_game' ) {
            ...
            for (let i = 0; i < getpoints.length; i++) {
                if (getpoints[i] >= winner) {
                    ...
                    answers = "Le jeu est terminé, le gagnant est " + winner_name + " avec le score de " + winner + " points. Les autres joueurs "+ getPlayers.filter(player => player !== winner_name) +" ont perdu avec des points successifs de " + getpoints.filter(player => player !== getpoints[i]) + " points."
